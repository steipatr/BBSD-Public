...
df_cl2t <- match.clusters(df_cl2c,"LCM",logging=TRUE)
df_cl3t <- match.clusters(df_cl3c,"True",logging=TRUE)
df_cl4t <- match.clusters(df_cl4c,"LCM",logging=TRUE)
...
for (k in 2:4){
  ...
  for (mthd in colnames(df_clusters_matched)){
    ...
    loc <- paste(getwd(),"/Report/Report/Figures/BudwormsClusteringSolutions/",Sys.Date(),"-",mthd,k,".png",sep="")

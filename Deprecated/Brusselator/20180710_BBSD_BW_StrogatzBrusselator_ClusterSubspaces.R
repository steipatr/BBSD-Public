...
df_expts_cl$cloi <- df_cl2m$DWT #cluster of interest
head(df_expts_cl)
...
clustersolutions_name <- "DWT_True"
...
#plot induced boxes with DWT-found memberships
SubspacesDWT_PRIM <- ggplot(df_expts_cl, aes(b0,a))+geom_point(aes(color=as.factor(cloi))) + 
  labs(color="Cluster") +
...
plot(SubspacesDWT_PRIM)

saveloc <- paste(getwd(),"/Report/Report/Figures/Brusselator/Subspaces/",Sys.Date(),"-DWT_PRIM.png",sep="")
ggsave(plot = SubspacesDWT_PRIM, saveloc, h = 4, w = 6, type = "cairo-png")

#plot induced boxes with DWT-found memberships
SubspacesDWT_PRIM_True <- ggplot(df_expts_cl, aes(b0,a))+geom_point(aes(color=as.factor(df_cl2$truecl))) + 
  labs(color="Cluster") +
  geom_rect(aes(xmin = 1.02, xmax = 4.08, ymin = 0.56, ymax = 1.98),color = 'darkred', alpha=0) +
  geom_rect(aes(xmin = 2.41, xmax = 4.98, ymin = 0.21, ymax = 1.11),color = 'darkblue', alpha=0)


plot(SubspacesDWT_PRIM_True)

saveloc <- paste(getwd(),"/Report/Report/Figures/Brusselator/Subspaces/",Sys.Date(),"-DWT_PRIM_True.png",sep="")
ggsave(plot = SubspacesDWT_PRIM_True, saveloc, h = 4, w = 6, type = "cairo-png")

brussfun <- function(x) sqrt(x-1)

SubspacesDWT_PRIM_TrueCurve <- ggplot(df_expts_cl, aes(b0,a))+geom_point(aes(color=as.factor(df_cl2$truecl))) + 
  labs(color="Cluster") +
  geom_rect(aes(xmin = 1.02, xmax = 4.08, ymin = 0.56, ymax = 1.98),color = 'darkred', alpha=0) +
  geom_rect(aes(xmin = 2.41, xmax = 4.98, ymin = 0.21, ymax = 1.11),color = 'darkblue', alpha=0) +
...
plot(SubspacesDWT_PRIM_TrueCurve)


saveloc <- paste(getwd(),"/Report/Report/Figures/Brusselator/Subspaces/",Sys.Date(),"-DWT_PRIM_TrueCurve.png",sep="")
ggsave(plot = SubspacesDWT_PRIM_TrueCurve, saveloc, h = 4, w = 6, type = "cairo-png")
